# Angler's Arrows v1.34.0 — deployment / file placement
...
```
<repo root>/
├── index.html                  ← REPLACE (v1.34.0)
├── service-worker.js           ← REPLACE (cache: anglers-arrows-1.34.0)
├── README.md                   ← REPLACE
...
└── assets/                     ← NEW folder (replaces images/ and tools/)
    ...
    ├── puzzles/                (no longer used as of v1.34.0 — safe to delete)
...
| `assets/puzzles/NN_slug_puzzle.jpg` | **unused since v1.34.0** — different painting from the cutout, never aligned |
...
2. Bottom bar should read **ARTEZIQ • Angler's Arrows • v1.34.0**.
